...
namespace UrlTracker.Core.Tests.Intercepting
{
    public class NoLongerExistsInterceptorTests : TestBase
    {
        ...
        {
            _testSubject = new NoLongerExistsInterceptor(ClientErrorRepository, new StaticUrlProviderCollection(new List<IStaticUrlProvider> { new StaticUrlProvider() }), new VoidLogger());
        }

        public static IEnumerable<TestCaseData> TestCases()
        {
            var result1 = new ClientErrorEntity("https://example.com", false, Core.Defaults.DatabaseSchema.ClientErrorStrategies.NoLongerExists);
            var result2 = new ClientErrorEntity("https://example.com", false, Core.Defaults.DatabaseSchema.ClientErrorStrategies.NotFound);
