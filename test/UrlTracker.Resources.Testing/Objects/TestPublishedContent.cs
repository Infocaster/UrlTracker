...
namespace UrlTracker.Resources.Testing.Objects
{
    ...
    {
        public static IPublishedContent Create(int id, PublishedItemType itemType = PublishedItemType.Content, Guid? key = null, bool addDefaultCulture = true)
        {
            var contentMock = new Mock<IPublishedContent>();
            contentMock.Setup(x => x.Id).Returns(id);
            contentMock.Setup(x => x.ItemType).Returns(itemType);
            contentMock.Setup(x => x.Key).Returns(key ?? Guid.NewGuid());
