...
{
    /// <summary>
    /// An implementation of <see cref="ITypedMemoryCache{TKey, TValue}"/> that implements thread-safe in-memory caching with guaranteed single creation.
    /// </summary>
    /// <typeparam name="TKey">The type by which objects are uniquely identified</typeparam>
        ...
        where TKey : notnull
    {
        ...
        public Task<TValue> GetOrCreateAsync(TKey key, Func<Task<TValue>> factory, MemoryCacheEntryOptions? options = null)
        {
            if (!_cache.TryGetValue(key, out Task<TValue> task))
            {
                ...
                var newTaskTask = new Task<Task<TValue>>(() => ExecuteFactoryAsync(factory, cts));
        ...
        [ExcludeFromCodeCoverage]
        private static async Task<TValue> ExecuteFactoryAsync(Func<Task<TValue>> factory, CancellationTokenSource cts)
        {
        ...
        /// <inheritdoc cref="TypedMemoryCache{TKey, TValue}.Dispose()"/>
        [ExcludeFromCodeCoverage]
