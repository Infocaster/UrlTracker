...
export class UrlTrackerSidebarSimpleRedirect extends LitElement {
  ...
  async save() {
    ...
    if (this.scope.model.id) {
      ...
      response = await this.redirectService.create({...this.redirectData, solvedRecommendation: this.$scope?.model.solvedRecommendation});
  ...
  protected render() {
    return html`<div class="header">${this.headerText}</div>
      <div class="main">
        <urltracker-create-simple-redirect .advancedView=${this.advancedView} .redirect=${this.redirectData} @update=${({detail}: {detail: IRedirectData}) => this.redirectData = detail }></urltracker-create-simple-redirect>
      </div>
      <div class="footer">
