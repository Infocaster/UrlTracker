...
export type IRecommendationCollection = IPagedCollectionResponseBase<IRecommendationResponse>;

export type IListRecommendationRequest = IPaginationRequestBase & IQueryRequestBase & IRecommendationFilterRequestBase;

export interface IRecommendationsService {
  list: (request: IListRecommendationRequest) => Promise<IRecommendationCollection>;

  update: (id: number, request: IRecommendationUpdate) => Promise<IRecommendationResponse>;

  updateBulk: (request: IRecommendationUpdateBulkRequest) => Promise<IRecommendationResponse[]>;
}
...
export class RecommendationsService implements IRecommendationsService {
  ...
  public async update(id: number, request: IRecommendationUpdate): Promise<IRecommendationResponse> {
    const response = await this.axios.post<IRecommendationResponse>(
  ...
  public async updateBulk(request: IRecommendationUpdateBulkRequest): Promise<IRecommendationResponse[]> {
    const response = await this.axios.post<IRecommendationResponse[]>(this.controller.getUrl('UpdateBulk'), request);
    return response.data;
  }
}
